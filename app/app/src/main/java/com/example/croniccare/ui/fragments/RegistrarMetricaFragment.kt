...
class RegistrarMetricaFragment : Fragment() {
    ...
    private fun registrar() {
        ...
        if (tipo == "presion") {
            ...
            if (valor !in 60.0..250.0) { binding.tilValor.error = "Sistólica debe estar entre 60 y 250 mmHg"; return }
            if (valorDiastolica !in 40.0..150.0) { binding.tilDiastolica.error = "Diastólica debe estar entre 40 y 150 mmHg"; return }
            binding.tilDiastolica.error = null
        }
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            val result = safeApiCall {
                RetrofitClient.instance.registrarMetrica(RegistrarMetricaRequest(tipo, valor, valorDiastolica))
            }
            ...
            when (result) {
                is ApiResult.Success -> {
                    ...
                    binding.etDiastolica.text?.clear()
                    showDialog(buildConfig(data.alerta, data.criticidad), valorDisplay, "$tipoLabel · $unidad")
                }
                is ApiResult.Error -> showDialog(errorConfig(), "—", "")
            }
        }
    }
    ...
    private fun buildConfig(isAlerta: Boolean, criticidad: String?): DialogConfig = when {
        !isAlerta -> DialogConfig(
            iconEmoji = "✓",
            circleBg  = AppColors.Ok.bg, circleStroke = AppColors.Ok.stroke, iconColor = AppColors.Ok.text,
            title = "¡Todo en orden!",
            chipLabel  = "Normal", chipBg = AppColors.Ok.bg, chipText  = AppColors.Ok.text,
            msg = "Tu valor está dentro del rango normal. Sigue así."
        )
        criticidad == "critica" -> DialogConfig(
            iconEmoji = "!",
            circleBg = AppColors.Critical.bg, circleStroke = AppColors.Critical.stroke, iconColor = AppColors.Critical.text,
            title = "¡Alerta generada!",
            chipLabel = "Crítico", chipBg = AppColors.Critical.bg, chipText  = AppColors.Critical.text,
            msg = "Se ha generado una alerta para tu médico. Consulta a tu médico cuanto antes."
        )
        else -> DialogConfig(
            iconEmoji = "!",
            circleBg = AppColors.Warning.bg, circleStroke = AppColors.Warning.stroke, iconColor = AppColors.Warning.text,
            title = "Valor elevado",
            chipLabel = if (criticidad == "moderada") "Moderado" else "Leve",
            chipBg = AppColors.Warning.bg, chipText  = AppColors.Warning.text,
    ...
    private fun errorConfig() = DialogConfig(
        ...
        chipLabel = "Sin conexión", chipBg = AppColors.Critical.bg, chipText  = AppColors.Critical.text,
